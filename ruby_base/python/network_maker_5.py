...
class Big:
    ...
    ### .tree contains modularity information via ids
        ...
        nx.write_pajek(G,'python/temporary_folder/' + file_name + '.net')   ### #change would have to change
    ...
    def tree_to_array(self, file_name):
        ...
        tree_text = open('python/temporary_folder/' + file_name +'.tree').readlines()    ### #change, should change dynamically
    ...
    def completer(self, file_name):
        ...
        node_net = open('python/temporary_folder/' + file_name +'.net').readlines()
        self.node_array = []
    ...
    def database_make(self, info_id):
        conn = lite.connect("db/development.sqlite3")

        cursor = conn.cursor()
            ###complete_array

        ### this code inputs the info_id (the :id of the query) into complete_array()
        for entry in self.complete_array:
            entry.append(info_id)

        ### this code creates a new table with complete_array, and then it addes in the info_id at the end.
        # cursor.execute("""CREATE TABLE Output (modules text, mys_a text, mys_b text, node_id int, word text, info_id int)""")
        cursor.executemany("INSERT INTO Output VALUES (?,?,?,?,?,?)", self.complete_array)

        # cursor.execute("""INSERT INTO info_groups (modules text, mys_a text, mys_b text, node_id int, word text, info_id int)""")
        # cursor.executemany("INSERT INTO Output VALUES (?,?,?,?,?,?)", self.complete_array)

        conn.commit()

        ### Print the below in case of creating a new table within the database
        print("\nHere's a listing of all the records in the table:\n")
        for row in cursor.execute("SELECT rowid, * FROM queries ORDER BY node_id"):
            print(row)

def network_main(subject, article_dump, info_id):
    # data_json = "vr.json"
